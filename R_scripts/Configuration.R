...
if(RUN_PART_PREPROCEESSING == RUN_CODE){
  ...
  LOW <- 'Low range of lipidomics, mz from 67 to 1000'
  HIGH <- 'High range of lipidomics, mz from 1000 to 2300'
  
  BLANK_NOISE_REMOVAL_ON <- 'Additional filtering after PP to remove blank and noise signals using blank samples'
  BLANK_NOISE_REMOVAL_OFF <- 'No additional filtering after PP to remove blank and noise signals using blank samples'
  ...
  CPC_FILTERING <- TRUE #CHOOSE TRUE OR FALSE
  LIPIDOMICS <- FALSE #CHOOSE LOW , HIGH OR FALSE
  BLANK_NOISE_REMOVAL <- BLANK_NOISE_REMOVAL_OFF
  PLOTTING <- SAVE_PLOTS
